...
/**
 * Motor de audio procedural del proyecto.
 *
 * <p>Encapsula la síntesis y reproducción de tonos, acordes y música de fondo.
 * {@link SoundManager} delega en esta clase la parte de DSP para mantener
 * separada la política de eventos del detalle de generación sonora.</p>
 */
public final class ProceduralAudioEngine {
    ...
    /**
     * Reproduce un blip con deslizamiento de frecuencia.
     *
     * @param startFrequency frecuencia inicial en Hz
     * @param endFrequency frecuencia final en Hz
     * @param durationMs duración total en milisegundos
     * @param volume volumen normalizado entre 0 y 1
     */
    public void playCartoonBlip(int startFrequency, int endFrequency, int durationMs, double volume) {
        Thread.ofPlatform().daemon(true).start(() ->
            playBuffer(buildGlideBuffer(startFrequency, endFrequency, durationMs, volume, true)));
    }

    /**
     * Reproduce un acorde breve con vibrato suave.
     *
     * @param frequencies notas del acorde en Hz
     * @param durationMs duración total en milisegundos
     * @param volume volumen normalizado entre 0 y 1
     * @param vibratoDepth profundidad relativa del vibrato
     */
    public void playCartoonChord(int[] frequencies, int durationMs, double volume, double vibratoDepth) {
        Thread.ofPlatform().daemon(true).start(() ->
            playBuffer(buildChordBuffer(frequencies, durationMs, volume, vibratoDepth)));
    }

    /**
     * Inicia la música continua si todavía no estaba sonando.
     */
    ...
    /**
     * Detiene la música continua y espera un cierre corto del hilo.
     */
    ...
    /**
     * Libera la reproducción continua asociada al motor.
     */
    public void close() {
        stopBackgroundMusic();
    }
    ...
    /**
     * Genera un buffer PCM para un blip con glide y envolvente cartoon.
     */
    private byte[] buildGlideBuffer(int startFrequency, int endFrequency, int durationMs,
                                    double volume, boolean bouncyEnvelope) {
    ...
    /**
     * Genera un buffer PCM para un acorde corto con vibrato leve.
     */
    ...
    /**
     * Mantiene la música de fondo en chunks cortos hasta que se solicite detenerla.
     */
    ...
    /**
     * Genera un fragmento de música combinando acorde, melodía y bajo.
     */
    ...
    /**
     * Envía un buffer PCM completo a un {@link SourceDataLine} temporal.
     */
